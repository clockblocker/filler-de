...
function formatParentBacklinkStr(basename: string, displayName: string): string {
...
function makeLeafBasename(nodeName: string, pathParts: string[]): string {
	...
	return makeJoinedSuffixedBasename({
		coreName: nodeName as ReturnType<typeof makeJoinedSuffixedBasename> extends string ? typeof nodeName : never,
