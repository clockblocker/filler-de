...
const createScrollLeaf = (
	...
	status:
		...
		| typeof TreeNodeStatusDeprecated.NotStarted = TreeNodeStatusDeprecated.NotStarted,
): TreeLeafDeprecated => ({
	extension: "md",
	nodeName,
...
describe("LibraryTreeD MoveNode", () => {
	...
	describe("creates parent path if needed", () => {
		it("creates intermediate sections", () => {
			const leaves: TreeLeafDeprecated[] = [createScrollLeaf("Note", ["A"])];
			const tree = new LibraryTreeDeprecated(leaves);
	...
	describe("updates section children chains", () => {
		it("updates children nodeNameChainToParent recursively", () => {
			...
			const sectionB = tree.getNode(["Library", "C", "B"]) as SectionNodeDeprecated;
	...
	describe("status recalculation", () => {
		it("recalculates old parent status after move", () => {
			...
			const leaves: TreeLeafDeprecated[] = [
				createScrollLeaf("DoneNote", ["A"], TreeNodeStatusDeprecated.Done),
				createScrollLeaf("NotStartedNote", ["A"], TreeNodeStatusDeprecated.NotStarted),
				createScrollLeaf("Other", ["B"]),
			];
			const tree = new LibraryTreeDeprecated(leaves);

			const sectionA = tree.getNode(["Library", "A"]) as SectionNodeDeprecated;
			expect(sectionA.status).toBe(TreeNodeStatusDeprecated.NotStarted);
			...
			const updatedA = tree.getNode(["Library", "A"]) as SectionNodeDeprecated;
			expect(updatedA.status).toBe(TreeNodeStatusDeprecated.Done);
		});
		...
		it("recalculates new parent status after move", () => {
			...
			const leaves: TreeLeafDeprecated[] = [
				createScrollLeaf("NotStartedNote", ["A"], TreeNodeStatusDeprecated.NotStarted),
				createScrollLeaf("DoneNote", ["B"], TreeNodeStatusDeprecated.Done),
			];
			const tree = new LibraryTreeDeprecated(leaves);

			const sectionB = tree.getNode(["Library", "B"]) as SectionNodeDeprecated;
			expect(sectionB.status).toBe(TreeNodeStatusDeprecated.Done);
			...
			const updatedB = tree.getNode(["Library", "B"]) as SectionNodeDeprecated;
			expect(updatedB.status).toBe(TreeNodeStatusDeprecated.NotStarted);
		});
	});
