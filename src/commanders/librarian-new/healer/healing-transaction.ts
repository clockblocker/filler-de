...
/**
 * Execute a healing transaction with automatic commit/rollback.
 *
 * @param healer - The healer instance
 * @param actions - Tree actions to apply
 * @param onDispatch - Callback to dispatch healing actions to vault
 * @returns Transaction summary
 */
export async function executeHealingTransaction(
	healer: Healer,
	actions: TreeAction[],
	onDispatch: (healingActions: HealingAction[]) => Promise<void>,
): Promise<Result<TransactionSummary, TransactionSummary>> {
	const tx = new HealingTransaction(healer);

	// Apply all tree actions
	...
	try {
		await onDispatch(tx.getHealingActions());
		tx.logSummary("debug");
		return ok(tx.commit());
	} catch (error) {
		tx.recordError(
