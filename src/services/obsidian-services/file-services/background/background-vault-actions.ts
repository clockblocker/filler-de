...
export const VaultActionType = LegacyVaultActionType;
export type VaultActionType = LegacyVaultActionType;

// Action payloads
export type LegacyVaultAction =
	| {
			type: typeof LegacyVaultActionType.UpdateOrCreateFolder;
			payload: { prettyPath: SplitPath };
	  }
	| {
			type: typeof LegacyVaultActionType.RenameFolder;
			payload: { from: SplitPath; to: SplitPath };
	  }
	| {
	...
	| {
			...
			payload: { prettyPath: SplitPathToMdFile; content?: string };
	  }
	| {
			type: typeof LegacyVaultActionType.RenameFile;
			payload: { from: SplitPathToMdFile; to: SplitPathToMdFile };
	  }
	| {
			type: typeof LegacyVaultActionType.TrashFile;
			payload: { prettyPath: SplitPathToMdFile };
	  }
	| {
...
export const weightForLegacyVaultActionType: Record<
...
export const weightForVaultActionType = weightForLegacyVaultActionType;

/**
 * Get a unique key for an action based on its target path.
 * Used for deduplication in the queue.
 */
export function getLegacyActionKey(action: LegacyVaultAction): string {
...
export function getActionKey(action: LegacyVaultAction): string {
	return getLegacyActionKey(action);
}

/**
 * Get the target path of an action (for logging/debugging).
 */
export function getLegacyActionTargetPath(action: LegacyVaultAction): string {
	const { type, payload } = action;
 ...
 */
export function sortLegacyActionsByWeight(
	...
	return [...actions].sort((a, b) => {
		const weightDelta =
			weightForLegacyVaultActionType[a.type] -
			weightForLegacyVaultActionType[b.type];
...
export function sortActionsByWeight(
	...
	return sortLegacyActionsByWeight(actions);
}
export type VaultAction = LegacyVaultAction;
