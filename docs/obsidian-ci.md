...
artifact deployment before launch, temporary Obsidian vault registration,
process startup by vault ID, readiness, serialized Bun tests, failure
diagnostics, and teardown. It removes its registry entry after Obsidian exits
and installs only Textfresser and the E2E driver in the generated vault. Every
scenario then owns `E2E/<session>/<scenario>/Library` and accesses it through
fixture, `act`, `snapshot`, and `status` operations.
