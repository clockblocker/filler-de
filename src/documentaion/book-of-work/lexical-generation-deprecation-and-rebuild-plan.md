...
4. Establish a narrow public entrypoint and keep internals private.
...
1. Implement `resolveSelection(...)`.
2. Implement `disambiguateSense(...)`.
3. Implement per-part generators.
4. Implement aggregated `generateLexicalInfo(...)` as orchestration over per-part generators.
5. Execute eligible part generations in parallel.
6. Support caller-level opt-out of individual parts.
